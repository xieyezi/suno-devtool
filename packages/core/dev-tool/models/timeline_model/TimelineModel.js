...
export class TimelineModelImpl {
  ...
  processGenericTrace(tracingModel) {
    ...
    for (const process of tracingModel.sortedProcesses()) {
      for (const thread of process.sortedThreads()) {
        this.processThreadEvents(tracingModel, [{ from: 0, to: Infinity }], thread, thread === browserMainThread, false, true, WorkletType.NotWorklet, null);
      }
    }
  }
  processMetadataAndThreads(tracingModel, metadataEvents) {
    ...
    for (let i = 0, length = metadataEvents.page.length; i < length; i++) {
      ...
      for (const thread of process.sortedThreads()) {
        ...
        this.processThreadEvents(tracingModel, [{ from: startTime, to: endTime }], thread, thread === metaEvent.thread, Boolean(workerUrl), true, WorkletType.NotWorklet, workerUrl);
      }
      startTime = endTime;
    }
  }
  processThreadsForBrowserFrames(tracingModel) {
    ...
    for (const process of tracingModel.sortedProcesses()) {
      ...
      let workletType = WorkletType.NotWorklet;
      ...
      for (const thread of process.sortedThreads()) {
        if (thread.name() === TimelineModelImpl.RendererMainThreadName) {
          this.processThreadEvents(tracingModel, ranges, thread, true, false, hasMain, WorkletType.NotWorklet, hasMain ? lastMainUrl : lastUrl);
          ...
          this.processThreadEvents(tracingModel, ranges, thread, false, true, false, WorkletType.NotWorklet, workerMetaEvent.args["data"]["url"] || Platform.DevToolsPath.EmptyUrlString);
        } else {
          let urlForOther = null;
          let workletTypeForOther = WorkletType.NotWorklet;
          if (thread.name() === TimelineModelImpl.AuctionWorkletThreadName || thread.name() === TimelineModelImpl.UtilityMainThreadName) {
          ...
          this.processThreadEvents(tracingModel, ranges, thread, false, false, false, workletTypeForOther, urlForOther);
        }
      }
    }
  }
  ...
  buildGPUEvents(tracingModel) {
    ...
    track.thread = thread;
    track.events = Root.Runtime.experiments.isEnabled("timelineShowAllEvents") ? thread.events() : thread.events().filter((event) => event.name === gpuEventName);
  }
  ...
  processBrowserEvent(event) {
    ...
    if (event.hasCategory(SDK.TracingModel.DevToolsMetadataEventCategory) && event.args["data"]) {
      ...
      if (event.name === TimelineModelImpl.DevToolsMetadataEvent.AuctionWorkletRunningInProcess && this.browserFrameTracking) {
        const worklet = new AuctionWorklet(event, data);
        this.auctionWorklets.set(data["target"], worklet);
      }
      if (event.name === TimelineModelImpl.DevToolsMetadataEvent.AuctionWorkletDoneWithProcess && this.browserFrameTracking) {
  ...
  reset() {
    ...
    this.auctionWorklets = /* @__PURE__ */ new Map();
...
export var RecordType = /* @__PURE__ */ ((RecordType2) => {
  ...
  RecordType2["EventTiming"] = "EventTiming";
...
((TimelineModelImpl2) => {
  ...
  TimelineModelImpl2.UtilityMainThreadName = "CrUtilityMain";
  TimelineModelImpl2.AuctionWorkletThreadName = "AuctionV8HelperThread";
...
export var TrackType = /* @__PURE__ */ ((TrackType2) => {
  ...
  TrackType2["UserInteractions"] = "UserInteractions";
  return TrackType2;
})(TrackType || {});
var WorkletType = /* @__PURE__ */ ((WorkletType2) => {
  WorkletType2[WorkletType2["NotWorklet"] = 0] = "NotWorklet";
  WorkletType2[WorkletType2["BidderWorklet"] = 1] = "BidderWorklet";
  WorkletType2[WorkletType2["SellerWorklet"] = 2] = "SellerWorklet";
  WorkletType2[WorkletType2["UnknownWorklet"] = 3] = "UnknownWorklet";
  return WorkletType2;
})(WorkletType || {});
...
export class AuctionWorklet {
  ...
  constructor(event, data) {
    ...
    this.host = typeof data["host"] === "string" ? data["host"] : void 0;
    this.startTime = event.startTime;
    this.endTime = Infinity;
    if (data["type"] === "bidder") {
      this.workletType = 1 /* BidderWorklet */;
    } else if (data["type"] === "seller") {
      this.workletType = 2 /* SellerWorklet */;
    } else {
      this.workletType = 3 /* UnknownWorklet */;
    }
  }
}
