...
export class IgnoreListManager {
  ...
  constructor(debuggerWorkspaceBinding) {
    ...
    Common.Settings.Settings.instance().moduleSetting("automaticallyIgnoreListKnownThirdPartyScripts").addChangeListener(this.patternChanged.bind(this));
    Common.Settings.Settings.instance().moduleSetting("enableIgnoreListing").addChangeListener(this.patternChanged.bind(this));
  ...
  async updateScriptRanges(script, sourceMap) {
    ...
    if (!IgnoreListManager.instance().isUserIgnoreListedURL(script.sourceURL, script.isContentScript())) {
      hasIgnoreListedMappings = sourceMap?.sourceURLs().some((url) => this.isUserOrSourceMapIgnoreListedURL(url, sourceMap.hasIgnoreListHint(url))) ?? false;
    }
    ...
    const newRanges = sourceMap.findRanges((srcURL) => this.isUserOrSourceMapIgnoreListedURL(srcURL, sourceMap.hasIgnoreListHint(srcURL)), { isStartMatching: true }).flatMap((range) => [range.start, range.end]);
  ...
  get automaticallyIgnoreListKnownThirdPartyScripts() {
    return this.enableIgnoreListing && Common.Settings.Settings.instance().moduleSetting("automaticallyIgnoreListKnownThirdPartyScripts").get();
  }
