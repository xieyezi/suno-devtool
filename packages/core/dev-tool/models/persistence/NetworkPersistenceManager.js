...
export class NetworkPersistenceManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async interceptionHandler(interceptedRequest) {
    ...
    if (fileSystemUISourceCode) {
      ...
      if (blob) {
        void interceptedRequest.continueRequestWithContent(new Blob([blob], { type: mimeType }), false, responseHeaders, true);
      }
    } else if (interceptedRequest.isRedirect()) {
      void interceptedRequest.continueRequestWithContent(new Blob([], { type: mimeType }), true, responseHeaders, false);
    } else {
      const responseBody = await interceptedRequest.responseBody();
      if (!responseBody.error && responseBody.content) {
        void interceptedRequest.continueRequestWithContent(new Blob([responseBody.content], { type: mimeType }), true, responseHeaders, false);
      }
    }
  }
}
