...
export class DeviceModeModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  modelAdded(emulationModel) {
    if (emulationModel.target() === SDK.TargetManager.TargetManager.instance().mainFrameTarget() && emulationModel.supportsDeviceEmulation()) {
  ...
  async captureScreenshot(fullSize, clip) {
    ...
    const screenshot = await screenCaptureModel.captureScreenshot(Protocol.Page.CaptureScreenshotRequestFormat.Png, 100, screenshotMode, clip);
    const deviceMetrics = {
      width: 0,
      height: 0,
      deviceScaleFactor: 0,
