...
export class ContrastRatioLineBuilder {
  ...
  drawContrastRatioLine(width, height, level) {
    ...
    let candidateLuminance = (candidateHSVA2) => {
      return Common.ColorUtils.luminance(Common.ColorUtils.blendColors(Common.Color.Legacy.fromHSVA(candidateHSVA2).rgba(), bgRGBA));
    };
    if (Root.Runtime.experiments.isEnabled("APCA")) {
      candidateLuminance = (candidateHSVA2) => {
        return Common.ColorUtils.luminanceAPCA(Common.ColorUtils.blendColors(Common.Color.Legacy.fromHSVA(candidateHSVA2).rgba(), bgRGBA));
      };
    }
    let s;
    for (s = 0; s < 1 + dS; s += dS) {
