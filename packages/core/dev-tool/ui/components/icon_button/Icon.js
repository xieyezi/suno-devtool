...
export class Icon extends HTMLElement {
  ...
  set data(data) {
    ...
    this.#width = isString(width)
      ? width
      : isString(height)
      ? height
      : this.#width;
    this.#height = isString(height)
      ? height
      : isString(width)
      ? width
      : this.#height;
    if ("iconPath" in data) {
      ...
      this.#iconPath = new URL(
        `../../../Images/${data.iconName}.svg`,
        import.meta.url
      ).toString();
  ...
  #render() {
    void coordinator.write(() => {
      LitHtml.render(
        LitHtml.html`
        <div class="icon-basic" style=${LitHtml.Directives.styleMap(
          this.#getStyles()
        )}></div>
      `,
        this.#shadow,
