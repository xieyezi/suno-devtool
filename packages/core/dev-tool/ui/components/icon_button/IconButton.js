...
export class IconButton extends HTMLElement {
  ...
  #render() {
    ...
    const filteredGroups = this.#groups
      .filter((counter) => counter.text !== void 0)
      .filter((_, index) => (this.#compact ? index === 0 : true));
    LitHtml.render(
      LitHtml.html`
      <button class=${buttonClasses} @click=${
        this.#onClickHandler
      } aria-label=${LitHtml.Directives.ifDefined(this.#accessibleName)}>
      ${
        !this.#compact && this.#leadingText
          ? LitHtml.html`<span class="icon-button-title">${
              this.#leadingText
            }</span>`
          : LitHtml.nothing
      }
      ...
      ${
        this.#compact
          ? LitHtml.html`<!-- Force line-height for this element --><span>&#8203;</span>`
          : LitHtml.nothing
      }
