  ...
  static async formatStyle(styleText, indentation, endIndentation) {
    ...
    function processToken(token, tokenType) {
      if (!insideProperty) {
        ...
        const isPropertyStart = tokenType?.includes("string") || tokenType?.includes("meta") || tokenType?.includes("property") || tokenType?.includes("variableName") && tokenType !== "variableName.function";
        if (disabledProperty) {
