...
export class ResponseHeaderSection extends HTMLElement {
  ...
  set data(data) {
    ...
    if (dataAssociatedWithRequest) {
      ...
      this.#headerEditors = this.#headerDetails.map((header) => ({ name: header.name, value: header.value, originalValue: header.value }));
      this.#markOverrides();
    }
  ...
  async #loadOverridesFileInfo() {
    ...
    try {
      ...
      this.#headersAreOverrideable = Root.Runtime.experiments.isEnabled(Root.Runtime.ExperimentName.HEADER_OVERRIDES) && Common.Settings.Settings.instance().moduleSetting("persistenceNetworkOverridesEnabled").get();
  ...
  #markOverrides() {
    ...
    const originalHeaders = /* @__PURE__ */ new Map();
    ...
    const actualHeaders = /* @__PURE__ */ new Map();
    for (const header of this.#headerDetails) {
    ...
    const isDifferent = (headerName, actualHeaders2, originalHeaders2) => {
      const actual = actualHeaders2.get(headerName);
      const original = originalHeaders2.get(headerName);
    ...
    this.#headerEditors.filter((header) => header.name === "set-cookie").forEach((header) => {
      if (this.#request?.originalResponseHeaders.find((originalHeader) => originalHeader.name === "set-cookie" && compareHeaders(originalHeader.value, header.value)) === void 0) {
        header.isOverride = true;
      }
    });
  }
