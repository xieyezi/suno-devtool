...
export const activeHints = /* @__PURE__ */ new WeakMap();
const UIStrings = {
...
export class StylePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  renderColorSwatch(text, valueChild) {
    ...
    if (!valueChild) {
      ...
      valueChild.textContent = color ? color.asString(swatch.getFormat() ?? void 0) : text;
    }
  ...
  updateAuthoringHint() {
    ...
    for (const validator of cssRuleValidatorsMap.get(propertyName) || []) {
      const hint = validator.getHint(propertyName, this.computedStyles || void 0, this.parentsComputedStyles || void 0, localName?.toLowerCase());
      if (hint) {
  ...
  startEditing(selectElement) {
    ...
    if (selectElement) {
      selectElement = selectElement.enclosingNodeOrSelfWithClass("webkit-css-property") || selectElement.enclosingNodeOrSelfWithClass("value") || selectElement.enclosingNodeOrSelfWithClass("styles-semicolon");
    }
    if (!selectElement) {
