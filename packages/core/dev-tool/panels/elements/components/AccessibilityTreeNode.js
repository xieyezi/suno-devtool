...
export class AccessibilityTreeNode extends HTMLElement {
  ...
  async #render() {
    ...
    const properties = this.#properties.map(({ name: name2, value }) => isPrintable(value.type) ? LitHtml.html` <span class='attribute-name'>${name2}</span>:&nbsp;<span class='attribute-value'>${value.value}</span>` : LitHtml.nothing);
    const content = this.#ignored ? LitHtml.html`<span>${i18nString(UIStrings.ignored)}</span>` : LitHtml.html`${role}&nbsp;${name}${properties}`;
