...
export class TimelineLoader {
  ...
  static async loadFromFile(file, client) {
    const loader = new TimelineLoader(client, true);
  ...
  static loadFromCpuProfile(profile, client, title) {
    const loader = new TimelineLoader(client, false, title);
    try {
      const events = TimelineModel.TimelineJSProfile.TimelineJSProfileProcessor.buildTraceProfileFromCpuProfile(profile, 1, true);
