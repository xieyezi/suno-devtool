...
export class StorageView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  static clear(target, securityOrigin, selectedStorageTypes, includeThirdPartyCookies) {
    ...
    if (set.has(Protocol.Storage.StorageType.Cache_storage) || hasAll) {
      const target2 = SDK.TargetManager.TargetManager.instance().mainFrameTarget();
  ...
  static clearByStorageKey(target, storageKey, originForCookies, selectedStorageTypes, includeThirdPartyCookies) {
    ...
    if (originForCookies && (set.has(Protocol.Storage.StorageType.Cookies) || hasAll)) {
      void target.storageAgent().invoke_clearDataForOrigin({ origin: originForCookies, storageTypes: Protocol.Storage.StorageType.Cookies });
      const cookieModel = target.model(SDK.CookieModel.CookieModel);
      if (cookieModel) {
...
export class ActionDelegate {
  ...
  handleClear(includeThirdPartyCookies) {
    ...
    if (storageKey) {
      StorageView.clearByStorageKey(target, storageKey, securityOrigin ? securityOrigin : void 0, AllStorageTypes, includeThirdPartyCookies);
