...
export class FrameDetailsView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  async doUpdate() {
    ...
    const debuggerModel = adScriptId?.debuggerId ? await SDK.DebuggerModel.DebuggerModel.modelForDebuggerId(adScriptId?.debuggerId) : null;
    const target = debuggerModel?.target();
    this.#reportView.data = { frame: this.#frame, target, prerenderedUrl: this.#prerenderedUrl, adScriptId: adScriptId || null };
  }
}
...
export class FrameDetailsReportView extends HTMLElement {
  ...
  #maybeRenderAdStatus() {
    ...
    const adScriptLinkElement = this.#target ? this.#linkifier.linkifyScriptLocation(this.#target, this.#adScriptId?.scriptId || null, Platform.DevToolsPath.EmptyUrlString, void 0, void 0) : null;
    return LitHtml.html`
