...
((SharedStorageTreeElementDispatcher2) => {
  let Events;
  ((Events2) => {
    Events2["SharedStorageTreeElementAdded"] = "SharedStorageTreeElementAdded";
...
export class ApplicationPanelSidebar extends UI.Widget.VBox {
  ...
  async addSharedStorage(sharedStorage) {
    ...
    this.sharedStorageTreeElementDispatcher.dispatchEventToListeners("SharedStorageTreeElementAdded" /* SharedStorageTreeElementAdded */, { origin: sharedStorage.securityOrigin });
  }
...
export class AppManifestTreeElement extends ApplicationPanelTreeElement {
  ...
  constructor(storagePanel) {
    ...
    this.view = new AppManifestView(emptyView, reportView, new Common.Throttler.Throttler(1e3));
...
export class IDBIndexTreeElement extends ApplicationPanelTreeElement {
  ...
  get itemURL() {
    return "indexedDB://" + this.databaseId.storageKey + "/" + this.databaseId.name + "/" + this.objectStore.name + "/" + this.index.name;
  }
  markNeedsRefresh() {
    if (this.view) {
      this.view.markNeedsRefresh();
