...
export class SourcesPanel extends UI.Panel.Panel {
  ...
  showFunctionDefinition(remoteObject) {
    void SDK.RemoteObject.RemoteFunction.objectAsFunction(remoteObject).targetFunction().then((targetFunction) => targetFunction.debuggerModel().functionDetailsPromise(targetFunction).then(this.didGetFunctionDetails.bind(this)));
  }
