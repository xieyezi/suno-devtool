...
export class DebuggerPlugin extends Plugin {
  ...
  shortcutHandlers() {
    ...
    return UI.ShortcutRegistry.ShortcutRegistry.instance().getShortcutListener({
      ...
      "debugger.breakpoint-input-window": async () => {
        ...
        Host.userMetrics.breakpointEditDialogRevealedFrom(Host.UserMetrics.BreakpointEditDialogRevealedFrom.KeyboardShortcut);
    ...
    if (!breakpoints.length) {
      if (this.editor && SourceFrame.SourceFrame.isBreakableLine(this.editor.state, line)) {
        ...
        if (supportsConditionalBreakpoints) {
          contextMenu.debugSection().appendItem(i18nString(UIStrings.addConditionalBreakpoint), () => {
            Host.userMetrics.breakpointEditDialogRevealedFrom(Host.UserMetrics.BreakpointEditDialogRevealedFrom.LineGutterContextMenu);
            this.editBreakpointCondition(line, null, null, false);
          });
          contextMenu.debugSection().appendItem(i18nString(UIStrings.addLogpoint), () => {
            Host.userMetrics.breakpointEditDialogRevealedFrom(Host.UserMetrics.BreakpointEditDialogRevealedFrom.LineGutterContextMenu);
            this.editBreakpointCondition(line, null, null, true);
          });
      ...
      if (breakpoints.length === 1 && supportsConditionalBreakpoints) {
        contextMenu.debugSection().appendItem(i18nString(UIStrings.editBreakpoint), () => {
          Host.userMetrics.breakpointEditDialogRevealedFrom(Host.UserMetrics.BreakpointEditDialogRevealedFrom.BreakpointMarkerContextMenu);
          this.editBreakpointCondition(line, breakpoints[0], null, false);
        });
      }
  ...
  populateTextAreaContextMenu(contextMenu) {
    ...
    if (this.uiSourceCode.project().type() === Workspace.Workspace.projectTypes.Network && Common.Settings.Settings.instance().moduleSetting("jsSourceMapsEnabled").get() && !Bindings.IgnoreListManager.IgnoreListManager.instance().isUserIgnoreListedURL(this.uiSourceCode.url())) {
      if (this.scriptFileForDebuggerModel.size) {
        ...
        if (scriptFile.script?.isWasm() && !Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().pluginManager?.hasPluginForScript(scriptFile.script)) {
  ...
  editBreakpointCondition(line, breakpoint, location, preferLogpoint) {
    ...
    function recordBreakpointWithConditionAdded(oldCondition2, newCondition) {
      const isLogpoint = newCondition.includes(LogpointPrefix);
      const isConditionalBreakpoint = newCondition.length !== 0 && !isLogpoint;
      const wasLogpoint = oldCondition2 && oldCondition2.includes(LogpointPrefix);
      const wasConditionalBreakpoint = oldCondition2 && oldCondition2.length !== 0 && !wasLogpoint;
      if (isLogpoint && !wasLogpoint) {
        ...
        Host.userMetrics.breakpointWithConditionAdded(Host.UserMetrics.BreakpointWithConditionAdded.ConditionalBreakpoint);
      }
    }
  }
  ...
  onInlineBreakpointMarkerContextMenu(event, breakpoint) {
    ...
    if (breakpoint) {
      contextMenu.debugSection().appendItem(i18nString(UIStrings.editBreakpoint), () => {
        Host.userMetrics.breakpointEditDialogRevealedFrom(Host.UserMetrics.BreakpointEditDialogRevealedFrom.BreakpointMarkerContextMenu);
        this.editBreakpointCondition(line, breakpoint, null, false);
      });
    } else {
      const uiLocation = this.transformer.editorLocationToUILocation(line.number - 1, position - line.from);
      contextMenu.debugSection().appendItem(i18nString(UIStrings.addConditionalBreakpoint), () => {
        Host.userMetrics.breakpointEditDialogRevealedFrom(Host.UserMetrics.BreakpointEditDialogRevealedFrom.BreakpointMarkerContextMenu);
        this.editBreakpointCondition(line, null, uiLocation, false);
      });
      contextMenu.debugSection().appendItem(i18nString(UIStrings.addLogpoint), () => {
        Host.userMetrics.breakpointEditDialogRevealedFrom(Host.UserMetrics.BreakpointEditDialogRevealedFrom.BreakpointMarkerContextMenu);
        this.editBreakpointCondition(line, null, uiLocation, true);
      });
  ...
  async setBreakpoint(lineNumber, columnNumber, condition, enabled) {
    ...
    await this.breakpointManager.setBreakpoint(this.uiSourceCode, lineNumber, columnNumber, condition, enabled, Bindings.BreakpointManager.BreakpointOrigin.USER_ACTION);
